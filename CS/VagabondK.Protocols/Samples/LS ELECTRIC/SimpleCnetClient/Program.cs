...
namespace SimpleCnetClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //var readIndividual = new CnetReadIndividualRequest(1) { "%DW0014" };
            var readContinuous = new CnetReadContinuousRequest(1, "%DW0014", count);
            ...
            while (true)
            {
                try
                {
                    ...
                    //var valuesIndividual = client.Read(1, "%DW0014");

                    var readContinuousResponse = client.Request(useBCC: false, readContinuous);
                    var valuesContinuous = client.Read(1, "%DW0014", count);
                    Console.WriteLine("");
                   // Console.WriteLine(JsonConvert.SerializeObject(valuesContinuous));
